...
export async function GET(req: NextRequest) {
  ...
  const code_verifier = store.get("pkce_v")?.value ?? "";
  ...
  if (!code || !code_verifier || !DOMAIN || !CLIENT || !REDIR) {
    return NextResponse.json({
      ...
      have: {
        code: !!code, client_id: !!CLIENT, redirect_uri: !!REDIR,
        domain: !!DOMAIN, code_verifier: !!code_verifier
      }
    }, { status: 400 });
  }
  ...
  // one-time PKCE cookie is no longer needed
  res.cookies.set("pkce_v", "", { ...base, maxAge: 0 });

  // optional convenience flag
  res.cookies.set("logged_in", "true", { ...base, maxAge: max });
